            ...
            <v-stepper-items>
              ...
              <v-stepper-content step="2">
                <v-card
                    min-height="200px"
                >
                  <p v-if="namespace">These are the users of namespace "{{namespace}}".</p>
              ...
              <v-stepper-content step="3">
                <v-card
                    ...
                    min-height="200px"
                >
                  <p v-if="namespace && selectedGroup">You have selected user "{{selectedGroup.name}}" of namespace "{{namespace}}".</p>
...
export default class AddUserDialogComponent extends MyVue {
  ...
  onNamespaceUpdate(namespace:string) {
    ...
    Container().getGroupService().getUnusedGroupListByNamespace(this.jobId, namespace).subscribe(result => {
      if(result == undefined) {
        ...
        console.log(result);
        this.availableGroupList = this.filterOutNonUserGroups(result);
   ...
   * @private
   */
  private filterOutNonUserGroups(groupList: Group[]): Group[]
  {
  ...
  save() {
    ...
    Container().getGroupService().addGroup(this.jobId, this.createGroupObject()).subscribe(result => {
      ...
      if(result === undefined) {
        Container().getErrorService().setError("Failed to add user!");
      } else {
        this.resetDialog();
        this.dialog = false; // close dialog
