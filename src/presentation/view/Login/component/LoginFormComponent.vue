...
export default class LoginFormComponent extends MyVue {
  ...
  updateNamespaceList() {
    Container()
      .getUserService()
      .listNamespacesByEmail(this.email)
      .subscribe((namespaceList: string[]) => {
  ...
  passwordReset() {
    return LoginFormType.TwoFactorAuth;
  }
  ...
  login(totp: string) {
    ...
    Container()
      .getUserService()
      .login(loginRequestData, this.rememberMe)
      .subscribe(loginResult => {
        if (loginResult.isTFAenabled) {
          this.tfaDialog = true;
          console.log(this.tfaDialog);
        } else if (loginResult.isAuthorized) {
